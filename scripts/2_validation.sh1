...
# - Extracts SHAs from branch-list API responses
# - Optionally validates commit counts for default branch plus first branches, max 10
# - Writes: validation-log-<date>.txt, validation-summary-<date>.csv, validation-summary-<date>.md
#
# CSV columns required:
#   project-key, project-name, repo, github_org, github_repo
#
# Env:
#   BBS_BASE_URL     : e.g. http://bitbucket.example.com:7990, or pass -b
#   BBS_USERNAME     : Bitbucket username
#   BBS_PASSWORD     : Bitbucket password
#   GH_TOKEN/GH_PAT  : GitHub token, or use gh auth login
#
# Optional Env:
#   BBS_DISABLE_SSL_VERIFY=true|Y|yes|1
#   COMMIT_CHECK=true|false
#   FAIL_ON_VALIDATION_FAILURES=true|false
#
# Usage:
...
ensure_tooling() {
  ...
  if ! command -v jq >/dev/null 2>&1; then
    log_error "jq is not installed. Please install jq before running this script."
    exit 1
  fi
...
# ---- Bitbucket Basic authentication using BBS_USERNAME and BBS_PASSWORD --------

auth_header() {
  if [[ -n "${BBS_USERNAME:-}" && -n "${BBS_PASSWORD:-}" ]]; then
    local b64
    b64="$(printf '%s:%s' "$BBS_USERNAME" "$BBS_PASSWORD" | base64 | tr -d '\n')"
    printf "Authorization: Basic %s" "$b64"
  else
    echo "[ERROR] Provide Bitbucket credentials using BBS_USERNAME and BBS_PASSWORD." >&2
    echo "Example:" >&2
    echo "  export BBS_USERNAME=\"your-bbs-username\"" >&2
    echo "  export BBS_PASSWORD=\"your-bbs-password\"" >&2
    exit 1
  fi
}
...
check_tls() {
  if $DISABLE_SSL_VERIFY; then
    log_warning "TLS certificate verification is DISABLED because BBS_DISABLE_SSL_VERIFY is set. Proceeding without certificate validation."
    return 0
  fi

  local probe rc
  probe="$(curl -sS -H "$(auth_header)" -o /dev/null "${BASE_URL}/rest/api/1.0/projects?limit=1" 2>&1)"
  rc=$?

  case "$rc" in
    35|51|58|59|60|66|77|83|91)
      log_error "TLS/SSL certificate validation failed for ${BASE_URL}. curl exit code: ${rc}. Details: ${probe}"
      log_error "If this host uses a self-signed or internal CA certificate intentionally, rerun with BBS_DISABLE_SSL_VERIFY=Y."
      exit 1
      ;;
  esac
...
check_bbs_auth() {
  ...
  http_code="$(
    curl "${CURL_OPTS[@]}" \
      ...
      -o "$body_file" \
      -w "%{http_code}" \
      "${BASE_URL}/rest/api/1.0/projects?limit=1" || true
  )"

  if [[ "$http_code" == "200" ]]; then
    log_success "Bitbucket authentication successful using BBS_USERNAME and BBS_PASSWORD."
    rm -f "$body_file"
    return 0
  fi

  log_error "Bitbucket authentication failed. HTTP status: ${http_code}"
  log_error "Response:"
...
get_bbs_branches_with_shas() {
  ...
  encProjectKey="$(urlencode_uri "$projectKey")"
  encRepoSlug="$(urlencode_uri "$repoSlug")"

  while :; do
    local resp
    resp="$(curl_json "${BASE_URL}/rest/api/1.0/projects/${encProjectKey}/repos/${encRepoSlug}/branches?limit=${limit}&start=${start}")"

    echo "$resp" | jq -r '.values[]? | [.displayId, .latestCommit] | @tsv'

    local isLast
    local nextStart
    isLast="$(echo "$resp" | jq -r '.isLastPage // true')"
...
get_bbs_default_branch() {
  ...
  curl_json "${BASE_URL}/rest/api/1.0/projects/${encProjectKey}/repos/${encRepoSlug}/branches/default" \
    2>/dev/null | jq -r '.displayId // empty' 2>/dev/null || true
}
...
get_bbs_commit_count() {
  ...
  while :; do
    local resp
    resp="$(curl_json "${BASE_URL}/rest/api/1.0/projects/${encProjectKey}/repos/${encRepoSlug}/commits?until=${encBranch}&limit=${limit}&start=${start}")"

    local cnt
    cnt="$(echo "$resp" | jq '.values | length' 2>/dev/null || echo 0)"
    [[ "$cnt" =~ ^[0-9]+$ ]] || cnt=0
    ...
    local isLast
    local nextStart
    isLast="$(echo "$resp" | jq -r '.isLastPage // true' 2>/dev/null)"
    nextStart="$(echo "$resp" | jq -r '.nextPageStart // empty' 2>/dev/null)"
...
gh_repo_exists() {
  gh api -X GET "/repos/$1/$2" >/dev/null 2>&1
}
...
get_gh_commit_count() {
  ...
  while :; do
    local count
    count="$(
      gh api "/repos/${org}/${repo}/commits?sha=${encBranch}&page=${page}&per_page=${per}" \
...
echo "Using Bitbucket auth: Basic authentication with BBS_USERNAME and BBS_PASSWORD"
...
if [[ ! -f "$CSV_PATH" ]]; then
  echo "[ERROR] CSV file not found: $CSV_PATH" | tee -a "$LOG_FILE"
  exit 1
fi

if [[ ! -s "$CSV_PATH" ]]; then
  echo "[ERROR] CSV has no rows: $CSV_PATH" | tee -a "$LOG_FILE"
  exit 1
fi
...
validate_repo() {
...

  {
    ...
    if ! gh_repo_exists "$ghOrg" "$ghRepo"; then
      echo "[$(date)] GitHub repo not found or inaccessible: ${ghOrg}/${ghRepo}. Treating GitHub side as empty."
      ghExists="no"
    fi
    ...
    local bbsBranchCount="${#bbsSHAmap[@]}"
    local ghBranchCount="${#ghSHAmap[@]}"
    local branchCountOk="false"

    [[ "$bbsBranchCount" -eq "$ghBranchCount" ]] && branchCountOk="true"
    ...
    if [[ -n "$missingInBBS" ]]; then
      echo "❌ Extra branches in GitHub not found in Bitbucket: $(echo "$missingInBBS" | tr '\n' ' ' | sed 's/ *$//')"
    else
      echo "✅ No extra branches found"
    fi

    local commitsMatchAll="false"
    local shasMatchAll="false"

    if [[ "$COMMIT_CHECK" != "true" ]]; then
      commitsMatchAll="true"
      shasMatchAll="true"
      echo "ℹ️ COMMIT_CHECK=false. Skipping per-branch commit and SHA comparison."
      ...
      bbsDefault="$(get_bbs_default_branch "$bbsProjectKey" "$bbsRepoSlug")"
      ...
      if (( bbsBranchCount > 10 || ghBranchCount > 10 )); then
        ...
        echo "ℹ️ Could not determine any branch for commit and SHA check."
      fi
    ...
    if [[ "$ghExists" == "no" ]]; then
      ...
      gh_notes="no branches on GitHub"
    fi
...
while IFS= read -r line; do
  ...
  if [[ -z "$bbsProjectKey" || -z "$bbsRepoSlug" || -z "$ghOrg" || -z "$ghRepo" ]]; then
    log_warning "Skipping row due to missing required values: $line"
    continue
  fi
...
for i in "${!PIDS[@]}"; do
  ...
  if [[ -f "$out" ]]; then
    grep -v '^CSV:' "$out" | tee -a "$LOG_FILE" || true
    grep '^CSV:' "$out" | sed 's/^CSV://' >> "$summary_csv" || true
    rm -f "$out"
  fi
done
...
if (( total_validated == 0 )); then
  ...
  awk -F',' '
    NR>1 && !($7=="true" && $8=="true" && $9=="true") {
      printf "::error::Validation discrepancy: %s/%s (%s)\n", $1, $2, $10
    }
  ' "$summary_csv"

else
  echo "::warning::Validation completed with discrepancies: ${passed} matched, ${failed} with issues out of ${total_validated}."

  awk -F',' '
    NR>1 && !($7=="true" && $8=="true" && $9=="true") {
      printf "::warning::Validation discrepancy: %s/%s (%s)\n", $1, $2, $10
    }
  ' "$summary_csv"
fi

if [[ "$FAIL_ON_VALIDATION_FAILURES" == "true" && "$failed" -gt 0 ]]; then
  echo "::error::FAIL_ON_VALIDATION_FAILURES=true and ${failed} repository or repositories failed validation."
  exit 1
fi
